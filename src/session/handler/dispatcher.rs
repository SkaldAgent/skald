...
use crate::db::{chat_llm_tools, chat_sessions_stack, stack_mcp_grants};
...
impl ChatSessionHandler {
    ...
    ///
    /// Called from a `tokio::spawn` task created by `dispatch_call_agent`.
    ...
    ///   1. Emits `AgentDone`
    ///   2. Completes (or fails) the parent's `call_agent` tool call
    ...
    pub(super) async fn run_child_frame(
        ...
        // Resume any pending tools from a previous crash, then run the child.
        ...
        // Clean up stack-scoped MCP grants regardless of outcome.
        ...
        match outcome {
            Ok(TurnOutcome::WaitingChild { child_stack_id: grandchild_id }) => {
                // This child itself spawned a grandchild asynchronously.
                // The grandchild task will cascade back up — nothing to do here.
                info!(
                    session_id  = self.session_id,
        ...
        // Drop the processing lock before calling resume_turn (which re-acquires it).
        drop(_guard);
        ...
        // Resume the parent turn. resume_turn will:
        //  - acquire the lock
        //  - find the parent stack via active_for_session
        //  - run the LLM loop so the parent sees the child's result
        //  - emit Done (or cascade further if the parent itself is a sub-agent)
