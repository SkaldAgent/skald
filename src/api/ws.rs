...
use crate::chat_hub::SendMessageOptions;
use crate::events::{ClientMessage, GlobalEvent, ServerEvent};
...
async fn handle_socket(mut socket: WebSocket, state: AppState, source: String) {
    let session_handler = match state.chat_hub.session_handler(&source).await {
        Ok(h)  => h,
        Err(e) => {
    ...
    let mut rx = state.chat_hub.events(&source);
        ...
        tokio::select! {
            ...
            msg = socket.recv() => {
                ...
                if is_resume_msg(&text) {
                    ...
                    let hub = Arc::clone(&state.chat_hub);
                ...
                if handle_approval_msg(&text, &state.chat_hub).await { continue; }
                if handle_question_answer_msg(&text, &session_handler).await { continue; }
                if handle_data_msg(&text, &state) { continue; }
                ...
                if cmd == "/sethome" {
                    let msg = match state.chat_hub.set_home(&source).await {
...
**/help** — questo messaggio"
                ...
                if cmd == "/context" {
                    match state.chat_hub.context_info(&source).await {
                        Ok((input, output)) => {
                ...
                if cmd == "/compact" {
                    match state.chat_hub.force_compact(&source).await {
                        Ok(true) => {
                            let _ = socket.send(to_msg(&ServerEvent::Done {
                                message_id:    0,
                                stack_id:      0,
                ...
                // ── Regular LLM message ───────────────────────────────────────
                let chat_hub = Arc::clone(&state.chat_hub);
                ...
                state.chat_hub.emit(GlobalEvent {
...
async fn handle_approval_msg(
    text:      &str,
    chat_hub:  &Arc<crate::chat_hub::ChatHub>,
) -> bool {
...
async fn handle_question_answer_msg(
    text:    &str,
    handler: &Arc<crate::session::handler::ChatSessionHandler>,
) -> bool {
    let Ok(v) = serde_json::from_str::<Value>(text) else { return false };
...
fn handle_data_msg(text: &str, state: &crate::server::AppState) -> bool {
    let Ok(v) = serde_json::from_str::<Value>(text) else { return false };
    if v["type"].as_str() != Some("data") { return false }

    let Ok(msg) = serde_json::from_value::<crate::events::InboundDataMessage>(v) else {
        return true;
    };
    ...
    match msg.stream.as_str() {
        "location" => {
            ...
            if let (Some(lat), Some(lng)) = (lat, lng) {
                state.location_manager.update(
                    "remote",
                    crate::location::GpsCoord { latitude: lat, longitude: lng },
                    acc,
                    live,
                );
