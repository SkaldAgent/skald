...
Capabilities:
  maps_directions    — transit/driving/walking directions from A to B
  maps_geocode       — convert an address or place name to coordinates
  maps_reverse_geocode — convert coordinates to an address
  maps_search_places — find nearby places (stations, stops, POIs)
  maps_distance_matrix — travel time & distance between multiple origins/destinations
...
def _maps_directions(args: dict) -> str:
    ...
    # Optional departure time: ISO string, Unix timestamp (int/float), or "now"
    departure_raw = args.get("departure_time", "now")
    if isinstance(departure_raw, (int, float)):
        # LLMs sometimes pass a Unix timestamp despite the schema saying string
        departure_time = datetime.fromtimestamp(departure_raw, tz=timezone.utc)
    elif departure_raw == "now":
    ...
    try:
        ...
        return f"Error: Directions API call failed: {e}"
...
def _maps_geocode(args: dict) -> str:
    ...
    try:
        ...
        return f"Error: Geocoding API call failed: {e}"
...
def _maps_reverse_geocode(args: dict) -> str:
    ...
    try:
        ...
        return f"Error: Reverse geocoding failed: {e}"
...
def _maps_search_places(args: dict) -> str:
    ...
    try:
        ...
        return f"Error: Places API call failed: {e}"
...
def _maps_distance_matrix(args: dict) -> str:
    ...
    try:
        ...
        return f"Error: Distance Matrix API call failed: {e}"
...
TOOL_DISPATCH = {
    "maps_directions":       _maps_directions,
    "maps_geocode":          _maps_geocode,
    "maps_reverse_geocode":  _maps_reverse_geocode,
    "maps_search_places":    _maps_search_places,
    "maps_distance_matrix":  _maps_distance_matrix,
}
