...
//!
//! Shared between the relay server and the mobile-connector plugin so both ends
//! serialize/deserialize the exact same wire format (see plugin.md §1.1).
...
///
/// `Serialize` is used by the relay server to emit them; `Deserialize` is used
/// by the mobile-connector plugin (the agent end) to receive them. Unknown
